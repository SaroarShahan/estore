...
class RoleController extends BaseController {
  ...
  async getAllRoles(req, res, next) {
    try {
      ...
      const roles = await roleServices.getAllRoles(req);
  ...
  async createRole(req, res, next) {
    try {
      ...
      if (!name) {
        ...
        const existingRole = await roleServices.roleRepository.findOne({ where: { name } });
        if (existingRole) {
          responseObj.httpStatusCode = 409;
          responseObj.message = 'Role with this name already exists';
        } else {
          responseObj.data = await roleServices.createRole({ name, permissions });
    ...
    try {
      ...
      if (!name) {
        ...
        if (existingRole && String(existingRole.id) !== String(role.id)) {
          ...
          responseObj.data = await roleServices.updateRole(req.params.id, { name, permissions });
          responseObj.httpStatusCode = 200;
